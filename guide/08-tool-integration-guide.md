...
┌─────────────────────────────────────────────────┐
│ 第一层：源文件 HEADER 元信息                     │
│  每个 guide/*.md 和 agents/*.md                  │
│  标注来源仓库和源文件路径                        │
│  AI 据此反查来源版本                             │
├─────────────────────────────────────────────────┤
│ 第二层：MANIFEST 注册表                          │
│  下游 docs/ai-engineering/MANIFEST.json          │
│  记录 guide/*.md 和 agents/*.md 的版本快照       │
├─────────────────────────────────────────────────┤
│ 第三层：AI 四步更新协议                          │
│  版本比对 → 策略选择 → 三路合并 → 验证           │
│  ★ 覆盖 guide/ 规范文件 + agents/ 角色定义       │
└─────────────────────────────────────────────────┘
```
...
| 配置方式 | 更新方式 | 说明 |
|----------|----------|------|
| `{file:...}` / `@path` 引用 | 无需重新部署 | 实时读取源文件，源文件更新即生效 |
| 直接嵌入（deploy-agents.sh） | 需重新执行部署 | 内容已复制，需重新嵌入或增量更新 |
...
| `agents.<name>.source` | 对应的 agents 源文件路径 |
| `agents.<name>.version` | 当前部署的 Agent 角色版本号 |
| `agents.<name>.customized` | 下游是否定制过 Agent 角色内容 |
| `agents.<name>.previous_version` | 更新前的版本号 |
...
步骤 1：版本比对
  ...
  │   包括 guide/*.md 和 agents/*.md
  ├─ 读取下游 MANIFEST.json，获取当前部署版本快照
  │   包括 files（规范文件）和 agents（角色定义）
  ├─ 逐条目比对 RELEASE.json vs MANIFEST.json 版本号
  └─ 标记过期清单（分两组：files 过期 / agents 过期）

步骤 2：策略选择（逐条目判定）
  ├─ files 组：guide/* → docs/ai-engineering/（复制/合并）
  │   ├─ 未在 MANIFEST 中记录（新增）→ 直接复制
  │   ├─ 未定制（customized: false）→ 覆盖源新版
  │   ├─ 已定制（customized: true）→ 三路合并
  │   └─ 已删除（MANIFEST 有记录但文件不存在）→ 跳过，标记异常
  ├─ agents 组：agents/* → subagent 配置（引用/嵌入）
  │   ├─ Git Submodule / {file:...} 引用 → 跳过（自动读取源文件）
  │   ├─ 直接嵌入且未定制 → 重新从源文件复制内容
  │   ├─ 直接嵌入且已定制 → 通知用户手动审查合并
  │   └─ agents 有更新但不可自动部署 → 输出更新摘要通知用户
  └─ 输出两份清单：规范文件更新项 / Agent 角色更新项

步骤 3：执行更新
  ├─ files 组三路合并（仅 customized: true 的规范文件）
  │   ├─ 输入：源旧版（Base）/ 源新版（Theirs）/ 下游定制版（Ours）
  │   └─ AI 自动合并，冲突无法自动解决 → 标记冲突行，挂起等待人工裁决
  └─ agents 组按策略执行
      ├─ 直接嵌入未定制 → 用源新版覆盖嵌入内容
      └─ 直接嵌入已定制 → 跳过，输出通知
...
2. 未定制文件 / agents（customized: false）→ 覆盖，风险最低
3. 已定制文件（customized: true）→ 三路合并，逐个处理
4. 已定制 agents（customized: true）→ 跳过，通知用户
5. 已删除文件 → 跳过并记录
6. 最后更新 MANIFEST.json（files + agents 版本）
```

> **agents 特殊说明**：直接嵌入的 Agent 角色文件不适合自动三路合并。agents 内容变更频率低，且定制通常是全局性行为（如增加项目特定规则），推荐人工审查。
...
Step 0：确认当前状态
  ...
  └─ 确定目标项目使用的 agent 部署方式
       ├─ Git Submodule / {file:...} / @path 引用 → agents 实时更新，无需部署
       └─ 直接嵌入模式（deploy-agents.sh） → agents 需增量更新

Step 1：获取源端版本快照
  ├─ 读取源仓库 RELEASE.json（一次获取所有文件最新版本）
  │   ★ 含 guide/*.md 和 agents/*.md 两组版本
  └─ 与下游 MANIFEST.json 逐条目比对，生成过期清单
       ├─ files 过期 → 规范文件更新
       └─ agents 过期 → 角色定义更新
...
Step 4：按策略更新 agents（仅直接嵌入模式）
  ├─ 引用模式（{file:...} / @path）→ 跳过，无需操作
  ├─ 直接嵌入未定制（customized=false）→ 重新从 agents/*.md 复制内容
  │   可复用 deploy-agents.sh --update 一键更新
  ├─ 直接嵌入已定制（customized=true）→ 输出变更摘要，通知人工审查
  └─ 嵌入文件本地已删除 → 标记异常

Step 5：三路合并操作（仅 customized=true 的文件）
...
Step 6：验证
  ├─ 检查文件结构完整（标题层级、列表闭合）
  ├─ 检查交叉引用有效（相对路径不断裂）
  ├─ 更新 MANIFEST.json：files 和 agents 的 version→新版号，previous_version→旧版号
  ├─ 如 agents 有更新，输出变更摘要通知用户
  └─ 通知 Orchestrator Agent 确认无异常
...
for fname, sver in src['files'].items():
    if not fname.startswith('guide/'): continue
...
print('=== Agent 角色 ===')
for aname, sver in src['files'].items():
    if not aname.startswith('agents/'): continue
    dver = dst['agents'].get(aname.replace('agents/', ''), {}).get('version', 'N/A') if 'agents' in dst else 'N/A'
    if sver != dver:
        print(f'{aname}: {dver} → {sver}')
"
...
print('=== Agent 角色（直接嵌入模式）===')
for name, info in m.get('agents', {}).items():
    if not info['customized']:
        print(f'# cp vendor/ai-engineering/{info[\"source\"]} → 重新嵌入 agent 文件')
"

# 5. 更新 MANIFEST 版本号（含 files + agents）
python3 -c "
import json
with open('vendor/ai-engineering/RELEASE.json') as s, \\
...
for aname, sver in src['files'].items():
    if not aname.startswith('agents/'): continue
    lname = aname.replace('agents/', '')
    if 'agents' in dst and lname in dst['agents'] and not dst['agents'][lname].get('conflict'):
        dst['agents'][lname]['previous_version'] = dst['agents'][lname]['version']
        dst['agents'][lname]['version'] = sver
...
| 版本 | 日期 | 修订内容 |
|------|------|----------|
| v0.7 | 2026-05-27 | §6 扩展更新范围至 subagent：更新架构概览、MANIFEST 注册表（新增 agents 字段）、四步更新协议（分组比对）、实战操作手册（Step 4 agents 更新）及命令速查 |
