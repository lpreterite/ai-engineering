...
- 人类确认后 → **此时加载** [references/comment-protocol.md](references/comment-protocol.md)，追加决策评论
- 决策结论同步更新正文「依赖/开放问题」或阻塞项

### 步骤 2.5：进度审查（人类发起时） [自由度：低]
- 人类说"看看进度"时触发
- 审查：读 Issue 状态 / 评论 / 复选框 / `.ai-engineering/drafts` 产出物
- **汇报**：当前状态 + 已确认事项 + 未决项 + 阻塞项勾选情况
...
- Task 拆分过度 → 合并回正文阻塞项，保留必要粒度
- 状态标签混乱 → `gh issue edit` 重置为真实状态
