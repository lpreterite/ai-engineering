...
使用以下命令完成规范文件和 Issue 模板的部署：

```bash
# 仅同步规范文件 + Issue 模板（Codex 无技能/Agent 自动部署机制）
mkdir -p docs/ai-engineering/
mkdir -p .github/ISSUE_TEMPLATE/
cp vendor/ai-engineering/guide/*.md docs/ai-engineering/
cp vendor/ai-engineering/.github/ISSUE_TEMPLATE/*.yml .github/ISSUE_TEMPLATE/
```
...
| 2 | 技能文件 | `skills/*/` | ❌ 无对应目录 | 内容已在 `developer_instructions` 中内联，更新需重新生成 agent 配置 |
| 3 | Agent 角色 | `agents/*.md` | `.codex/agents/*.toml` | 不支持文件引用，需重新粘贴到 `developer_instructions` |
| 4 | 主指令文件 | `reference/templates/codex-AGENTS.md.example` | `AGENTS.md` | 手动更新占位符 |
...
```bash
# 同步组件 1、5（规范文件 + Issue 模板），不覆盖已有文件
mkdir -p docs/ai-engineering/
mkdir -p .github/ISSUE_TEMPLATE/
cp -n vendor/ai-engineering/guide/*.md docs/ai-engineering/
cp -n vendor/ai-engineering/.github/ISSUE_TEMPLATE/*.yml .github/ISSUE_TEMPLATE/
```

Codex CLI 不支持 Agent 角色和技能的自动同步，组件 2-3 需手动处理。
...
| Agent 角色 | 手动将新版本 `agents/*.md` 内容粘贴到 `.codex/agents/*.toml` 的 `developer_instructions` |
...
□ 规范文件版本已更新
□ AGENTS.md 已同步
□ Agent 配置中的 developer_instructions 已更新为最新角色定义
```
